...
- **抽取简历画像**：目标岗、技能标签、城市、学历、政治面貌、经验年限、专业。
- **构造检索式**：组合「岗位关键词 + 地区 + 2026 校招/社招/应届 + 国企/事业单位/民企」，用**联网搜索**检索主流招聘平台公开岗位。
- **🔒 硬性条件过滤（强制门禁，先于评分）**：逐条核对候选岗位的应届届数/择业期、学历、经验年限、政治面貌、专业、地域。**任一项不满足 → 直接剔除，不推荐**（如要求「2027 届」/「硕士」/「3–5 年经验」而候选人不满足）。仅"身份可能符合但报名状态未明确"的岗可单列"待核实"小节，不得作为正式推荐。
- **筛选排序**：仅保留真实在招、通过硬条件、与画像匹配度 ≥ 阈值者，按匹配度排序。
- **输出推荐清单**：每条含「岗位 + 单位类型 + 地区 + 匹配度 + 为什么匹配 + 简历微调建议 + 投递渠道」。
- **生成推荐电子表格（可选交付物）**：当用户要"整理成表/生成表格"时，按 `references/job-matching.md` 第 9 步把结果落成 xlsx/csv，字段含「企业/单位、岗位、工作地址、要求条件、条件相符度、报名状态、报名/截止时间、报名方式、报名网址、匹配理由、备注」，按"报名中→即将开始→待核实"排序，条件不符者一律不进表。
